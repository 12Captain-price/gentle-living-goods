...
import { LOGO_DATA_URI, LOGO_ASPECT_RATIO } from "@/lib/logo";
...
function generateDocPdf(input: DocPdfInput) {
  ...
  // Logo — sized to sit inside the header band without crowding the
  // company address text next to it.
  const logoH = 20;
  const logoW = logoH * LOGO_ASPECT_RATIO;
  doc.addImage(LOGO_DATA_URI, "PNG", 14, 5, logoW, logoH);

  const textX = 14 + logoW + 6;
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.text("Stationery, Office & ICT Solutions", textX, 12);
  doc.setFont("helvetica", "normal");
  doc.text("Shop R4 Main Street Mall, Between 14th & 15th Ave", textX, 18);
  doc.text("J. Nkomo Street, Bulawayo · +263 77 409 8174 / +263 77 586 3002", textX, 23);
