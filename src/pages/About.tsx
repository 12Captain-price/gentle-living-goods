...
import { Editable, useContent } from "@/lib/site-content";
import { useAboutGallery, useAboutLogos } from "@/lib/admin-data";

const About = () => {
  const [gallery] = useAboutGallery();
  const [logos] = useAboutLogos();

  const [heroImage] = useContent("about.hero.image", "https://images.unsplash.com/photo-1497215728101-856f4ea42174?w=1920&q=80");
  const [story1Image] = useContent("about.story1.image", "https://images.unsplash.com/photo-1568234928966-359c35dd8327?w=1200&q=80");
  const [bannerImage] = useContent("about.banner.image", "https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=1920&q=80");
  const [story2Image] = useContent("about.story2.image", "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1200&q=80");
  const [strip1Image] = useContent("about.strip1.image", "https://images.unsplash.com/photo-1513364776144-60967b0f800f?w=800&q=80");
  const [strip2Image] = useContent("about.strip2.image", "https://images.unsplash.com/photo-1497215728101-856f4ea42174?w=800&q=80");
  const [strip3Image] = useContent("about.strip3.image", "https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=800&q=80");
  const [ctaImage] = useContent("about.cta.image", "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=1920&q=80");
    ...
    <Layout>
      ...
      <section className="py-28 md:py-40">
        <div className="container-narrow">
          ...
          >
            ...
            <Editable
              as="h2"
              className="font-serif text-3xl md:text-4xl lg:text-5xl text-foreground leading-[1.25] tracking-tight block"
              k="about.philosophy"
              defaultValue="We believe the right pen, the right notebook and the right file can make school easier, work sharper and ideas travel further."
            />
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-4 md:py-6">
        <div className="grid grid-cols-3 gap-2 md:gap-4 h-[35vh] md:h-[50vh]">
          {[
            { src: strip1Image, alt: "Colour pencils and art supplies" },
            { src: strip2Image, alt: "Tidy office desk with stationery" },
            { src: strip3Image, alt: "An open notebook" },
          ].map((img, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
      ...
      {/* Gallery — admin-managed, only shown once images have been added */}
      {gallery.length > 0 && (
      ...
      {/* Brand Logos — admin-managed, only shown once logos have been added */}
      {logos.length > 0 && (
        <section className="py-16 md:py-24 bg-linen">
          <div className="container-full">
            <p className="text-center text-[11px] font-semibold tracking-[0.3em] uppercase text-primary mb-10">
              Brands We Stock
            </p>
            <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-8">
              {logos.map((l, i) => {
                const img = (
                  <img
                    src={l.src}
                    alt={l.name || "Brand logo"}
                    className="h-10 md:h-12 object-contain grayscale opacity-70 hover:grayscale-0 hover:opacity-100 transition-all duration-300"
                  />
                );
                return l.url ? (
