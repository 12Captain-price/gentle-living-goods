...
import {
  ...
  useAdminProducts, useCategories, useAboutGallery, useAboutLogos,
  Zone, Coupon,
} from "@/lib/admin-data";
import { useQuotations, useInvoices, downloadQuotationPdf, downloadInvoicePdf, Quotation, Invoice, DocItem } from "@/lib/quotations";
import { useEditMode, useContent } from "@/lib/site-content";
import { uploadProductImage } from "@/lib/supabase";
import { randomId } from "@/lib/uuid";
import { Product, fmtUsd, fmtZwl, collections } from "@/data/products";
...
function ProductsTab() {
  ...
  return (
    <div>
      ...
      {(editing || showAdd) && (
        <ProductModal
          initial={editing || { id: randomId(), name: "", slug: "", collection: categories[0]?.id || "", price: 0, stock: 0,
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-background max-w-2xl w-full max-h-[90vh] overflow-y-auto p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid gap-4">
          ...
          <Field label="Stock"><Input type="number" value={p.stock} onChange={(e) => setP({ ...p, stock: parseInt(e.target.value) || 0 })} className="rounded-none max-w-[160px]" /></Field>
          <Field label="Images">
            <ProductImagesField
              images={p.images}
...
// Uploads a File from the user's device to Supabase Storage and hands
// back its public URL, so product photos can be uploaded directly
// instead of only pasted as a URL. (See src/lib/supabase.ts)

function ProductImagesField({ images, onChange }: { images: string[]; onChange: (images: string[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={addBlank} className="flex items-center gap-1 text-xs underline text-muted-foreground hover:text-foreground">
        <Plus className="w-3 h-3" /> Add another image
      </button>
      <p className="text-[11px] text-muted-foreground">First image is used as the main product photo.</p>
    </div>
  );
}

function ImageSlot({ index, value, onChange, onRemove }: {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      console.error("Zozo: image upload failed", err);
      alert("Image upload failed. Check your Supabase Storage bucket & policies.");
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      {value ? (
        <img src={value} alt="" className="w-12 h-12 object-cover border border-border shrink-0" />
      ) : (
        <div className="w-12 h-12 border border-dashed border-border shrink-0" />
      )}
      <Input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={index === 0 ? "Main image URL" : "Image URL"}
      ...
      />
      <Button type="button" variant="outline" size="sm" className="rounded-none shrink-0" onClick={() => fileRef.current?.click()} disabled={uploading}>
        <Upload className="w-3.5 h-3.5 mr-1" /> {uploading ? "Uploading…" : "Upload"}
      </Button>
      {onRemove && (
        <button type="button" onClick={onRemove} className="text-destructive shrink-0" aria-label="Remove image">
...
const CONTENT_FIELDS = [
  ...
  { key: "home.featured.description", label: "Homepage — featured collection blurb", def: "From the trusted BIC Cristal to the iconic Parker Jotter — find a pen that suits the way you think, work and sign." },
  { key: "home.about.body", label: "Homepage — about headline", def: "For over a decade, Stationery City has kept Bulawayo's classrooms, boardrooms and studios stocked — one honest pen, notebook and ream at a time." },
  { key: "home.collections.eyebrow", label: "Homepage — collections eyebrow", def: "Browse By" },
  { key: "home.collections.title", label: "Homepage — collections title", def: "Collections" },
...
const IMAGE_FIELDS = [
  { key: "home.hero.image", label: "Homepage — hero background", def: "https://images.unsplash.com/photo-1455390582262-044cdead277a?w=1920&q=80" },
  { key: "home.featured.image", label: "Homepage — featured collection image", def: featuredCollection.heroImage || featuredCollection.image },
  { key: "about.hero.image", label: "About — hero background", def: "https://images.unsplash.com/photo-1497215728101-856f4ea42174?w=1920&q=80" },
  { key: "about.story1.image", label: "About — \"A Shop Built for Bulawayo\" image", def: "https://images.unsplash.com/photo-1568234928966-359c35dd8327?w=1200&q=80" },
  { key: "about.banner.image", label: "About — full-width quote banner image", def: "https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=1920&q=80" },
  { key: "about.story2.image", label: "About — \"From Our Counter\" image", def: "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1200&q=80" },
  { key: "about.strip1.image", label: "About — three-image strip, image 1", def: "https://images.unsplash.com/photo-1513364776144-60967b0f800f?w=800&q=80" },
  { key: "about.strip2.image", label: "About — three-image strip, image 2", def: "https://images.unsplash.com/photo-1497215728101-856f4ea42174?w=800&q=80" },
  { key: "about.strip3.image", label: "About — three-image strip, image 3", def: "https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=800&q=80" },
  { key: "about.cta.image", label: "About — CTA background image", def: "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=1920&q=80" },
];
...
function ContentTab() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-6">Edit text and images directly below — changes go live immediately.</p>
      ...
      <label className="inline-flex items-center gap-3 mb-6 cursor-pointer">
        ...
        <span className="text-sm font-medium">Click-to-edit text on storefront {editMode ? "ON" : "OFF"}</span>
      </label>

      <h2 className="font-serif text-xl mb-3">Text</h2>
      <div className="bg-background border border-border divide-y divide-border mb-10">
        {CONTENT_FIELDS.map((f) => <ContentRow key={f.key} field={f} />)}
      </div>

      <h2 className="font-serif text-xl mb-3">Images</h2>
      <p className="text-sm text-muted-foreground mb-4">Paste a URL or upload a photo from your device — updates immediately on the live site.</p>
      <div className="bg-background border border-border divide-y divide-border">
        {IMAGE_FIELDS.map((f) => <ImageContentRow key={f.key} field={f} />)}
      </div>
    </div>
  );
}
...
function ContentRow({ field }: { field: { key: string; label: string; def: string } }) {
  const [val, setVal] = useContent(field.key, field.def);
  ...
  return (
    <div className="p-4 grid md:grid-cols-3 gap-3 items-start">
      ...
      <Textarea value={draft} onChange={(e) => setDraft(e.target.value)} className="rounded-none md:col-span-2" />
      <div className="md:col-span-3 flex gap-3 justify-end">
        <button onClick={() => { setVal(field.def); setDraft(field.def); }} className="text-xs underline">Reset</button>
        <button onClick={() => setVal(draft)} className="text-xs bg-navy text-white px-4 py-1">Save</button>
      </div>
    </div>
  );
}

function ImageContentRow({ field }: { field: { key: string; label: string; def: string } }) {
  const [val, setVal] = useContent(field.key, field.def);
  const [draft, setDraft] = useState(val);
  const fileRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  ...
  return (
    <div className="p-4 grid md:grid-cols-3 gap-3 items-start">
      ...
      <div className="md:col-span-2 flex items-center gap-2">
        {val ? (
          <img src={val} alt="" className="w-16 h-16 object-cover border border-border shrink-0" />
        ) : (
          <div className="w-16 h-16 border border-dashed border-border shrink-0" />
        )}
        <Input
          value={draft}
...
// -------- About Page
function AboutTab() {
  const [gallery, setGallery] = useAboutGallery();
  const [logos, setLogos] = useAboutLogos();
